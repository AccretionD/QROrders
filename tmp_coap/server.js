...
function CoAPServer(options, listener) {
  ...
  this.cache=  new CachemanRedis('redis://127.0.0.1:6379')
...
CoAPServer.prototype._sendError = function(payload, rsinfo, packet) {
  ...
  this.sock.send(message, 0, message.length, rsinfo.port)
}
...
CoAPServer.prototype._sendProxied = function(packet, proxyUri, callback) {
  ...
  this.sock.send(message, 0, message.length, port, host, callback)
}
...
CoAPServer.prototype.listen = function(port, address, done) {
  ...
  this.sock = dgram.createSocket(this._options.type, handleRequest(this))

  this.sock.on('error', function(error) {
    that.emit('error', error)
  })

  this.sock.bind(port, address || null, done || null)
  this._port = port
...
CoAPServer.prototype._handle = function(packet, rsinfo) {
  ...
  var sock      = this.sock
    ...
    , acks      = this._acks
    ,cache=this.cache
    , cached2    = lru.peek(toKey(rsinfo.address, rsinfo.port, packet, true))
    ,cached = cache.get(toKey(rsinfo.address, rsinfo.port, packet, true))
    , Message   = OutMessage
    , that = this
    , request
    , response

  if (cached2 && !packet.ack && !packet.reset)
    return sock.send(cached2, 0, cached2.length, rsinfo.port, rsinfo.address)
  else if (cached2 && (packet.ack || packet.reset)) {
    if (cached2.response && packet.reset)
      cached2.response.end()
      cached.del(toKey(rsinfo.address, rsinfo.port, packet, false))
  ...
  response = new Message(packet, function(response, packet) {
    ...
    if (Message === OutMessage) {
      ...
      // if(pgk)
       if(request.headers['Observe'] === 0){
	  info_cli={address:rsinfo.address,port:rsinfo.port,msgId:pkg.messageId,token:pkg.token};
	  //info_cli={info:rsinfo,buffer:pgk};
	console.log('cache save data... ',pkg.options,info_cli);

    	  cache.set(toKey(rsinfo.address, rsinfo.port,
         packet, false), info_cli,10000, function (err, value) {
  	if (err) throw err;
    	})
